    ...
    def generate_markdown_content(self, post_data):
        ...
        if post_data['topics']:
            ...
            for topic in post_data['topics']:
                ...
                # Match speaker to topic (by index, with fallback for single speaker)
                if post_data['speakers']:
                    # If one speaker for multiple topics, use the same speaker for all
                    if len(post_data['speakers']) == 1:
                        speaker_name = post_data['speakers'][0]['name']
                    # Otherwise use index-based mapping
                    elif topic['number'] - 1 < len(post_data['speakers']):
                        speaker_name = post_data['speakers'][topic['number'] - 1]['name']
                    else:
                        # Fallback to last speaker if we run out
                        speaker_name = post_data['speakers'][-1]['name']
